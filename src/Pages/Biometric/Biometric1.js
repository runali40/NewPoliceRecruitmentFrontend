...
import { CaptureFinger } from "./Mfs110";
...
const Biometric1 = () => {
  ...
  const key = CryptoJS.enc.Hex.parse(secretKey);   // 🔑 same key
  const iv = CryptoJS.lib.WordArray.random(16);    // 🎯 random IV

  const encrypted = CryptoJS.AES.encrypt(template, key, {
    iv,
...
const CaptureFingerHere1 = async () => {
  try {
    ...
    // 1️⃣ Capture
    const result = await CaptureFinger(60, 10000);
    ...
    if (!resp || resp.getAttribute("errCode") !== "0") {
      toast.dismiss(loadingToast);
      toast.error(resp?.getAttribute("errInfo") || "Capture error");
      return;
    }

    // 4️⃣ Get TEMPLATE (NOT IMAGE)
    const dataNode = xmlDoc.getElementsByTagName("Data")[0];
    const template = dataNode?.textContent;

    if (!template) {
      toast.dismiss(loadingToast);
      toast.error("Fingerprint template missing");
      return;
    }
    ...
    // 6️⃣ Encrypt template
    const encrypted = encryptTemplate(template);

    // 7️⃣ Save encrypted template
    setFingerImage1(encrypted);

    toast.dismiss(loadingToast);
    toast.success(`Fingerprint captured (Quality: ${qScore})`);
...
useEffect(() => {
  if (!fingerImage1) return;

  const decrypted = decryptTemplate(fingerImage1);
  console.log("DECRYPTED TEMPLATE:", decrypted);
  ...
  //   }
  // };
  const CaptureFingerHere3 = async () => {
    try {
      // Show loading toast
      const loadingToast = toast.loading("Capturing fingerprint 3...");
      ...
      console.log("Encrypted photo 3:", encryptedFingerSrc);
      ...
      // Dismiss loading and show success
      toast.dismiss(loadingToast);
      toast.success("Finger 3 captured successfully!");
      ...
      console.error("Error during fingerprint 3 capture:", error);
      toast.error("Error: " + (error.message || "Failed to capture fingerprint 3"));
      return null;
    }
  };
  //capture 4//
  ...
  //   }
  // };
  const CaptureFingerHere4 = async () => {
    try {
      // Show loading toast
      const loadingToast = toast.loading("Capturing fingerprint 4...");
      ...
      console.log("Encrypted photo 4:", encryptedFingerSrc);
      ...
      // Dismiss loading and show success
      toast.dismiss(loadingToast);
      toast.success("Finger 4 captured successfully!");
      ...
      console.error("Error during fingerprint 4 capture:", error);
      toast.error("Error: " + (error.message || "Failed to capture fingerprint 4"));
      return null;
    }
  }
  ...
  const CaptureFingerHere5 = async () => {
    try {
      // Show loading toast
      const loadingToast = toast.loading("Capturing fingerprint 5...");
      ...
      console.log("Encrypted photo 5:", encryptedFingerSrc);
      ...
      // Dismiss loading and show success
      toast.dismiss(loadingToast);
      toast.success("Finger 5 captured successfully!");
      ...
      console.error("Error during fingerprint 5 capture:", error);
      toast.error("Error: " + (error.message || "Failed to capture fingerprint 5"));
      return null;
    }
  };
