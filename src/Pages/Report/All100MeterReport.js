...
import { fetchAll100Meter, GetCategory } from '../../Components/Api/DailyReportApi'
...
const All100MeterReport = () => {
  ...
  const Get100MeterData = async () => {
    const data = await fetchAll100Meter(eventId, groupId);
    console.log(data)
  ...
  const handleGroup = async (selected) => {
    ...
    const data = await fetchAll100Meter(eventId, selectedValue.value);
    console.log(data)
    setAll100MeterReport(data)
  }
